...
class CashFlowService extends Service {
  ...
  static getAllEntries(query: CashFlow.Query) {
    ...
    return this.Http
      .get<CashFlow.EntrySummary[]>(`/cashflow/entries${queryString}`)
      .then(this.getData);
  }
  ...
  static getAllCategories(
    query?: { sort: [keyof CashFlow.CategorySummary, 'asc' | 'desc'] }
  ) {
    let queryString = ''
    if (query)
      queryString = generateQueryString(query);
    return this.Http
      .get<CashFlow.CategorySummary[]>(`/cashflow/categories${queryString}`)
      .then(this.getData);
  }
